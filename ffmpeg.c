    ...
    if (need_reinit || !fg->graph) {
        ...
        for (i = 0; i < fg->nb_inputs; i++) {
            while (av_fifo_size(fg->inputs[i]->frame_queue)) {
                ...
                ret = av_buffersrc_add_frame_flags(fg->inputs[i]->filter, tmp, AV_BUFFERSRC_FLAG_PUSH);
                av_frame_free(&tmp);
                if (ret < 0)
                    return ret;
            }
...
static int check_init_output_file(OutputFile *of, int file_index)
{
    ...
    for (i = 0; i < of->ctx->nb_streams; i++) {
        ...
        /* Refresh the streamcopy timebase, the muxer might have changed it */
        if (ost->stream_copy)
...
static int init_output_stream(OutputStream *ost, char *error, int error_len)
{
    ...
    if (ost->encoding_needed) {
        ...
        // copy timebase while removing common factors
        ost->mux_timebase  = ost->enc_ctx->time_base;
...
{
    int i, ret = AVERROR(EAGAIN);
    int nb_requests, nb_requests_max = 0;
    InputFilter *ifilter;
    InputStream *ist;

    *best_ist = NULL;
    /* query the graph if its already up and running */
    if (graph->graph) {
    ...
    for (i = 0; i < graph->nb_inputs; i++) {
        ...
        /* if the filter is not initialized yet, then give it high priority */
        if (ifilter->filter)
